...
function setAnimationFunction(view) {
	paper.view.onFrame = function(event) {
		...
		if (simulationRunning) {
			if (
				engine.timing.timestamp - startTime < parseFloat(endTime) ||
				endTime == 0
			) {
				//slightly noise
				for (var i = 0; i < concreteObjects.length; i++) {
					//console.log
					var size = [
						physicObjects[i].bounds.max.x -
							physicObjects[i].bounds.min.x,
						physicObjects[i].bounds.max.y -
							physicObjects[i].bounds.min.y
					];
					var physicbounds = new Rectangle(
						physicObjects[i].bounds.min,
						size
					);
					if (!physicObjects[i].isSleeping) {
						concreteObjects[i].applyMatrix = true;
						concreteObjects[i].position = physicbounds.center;
						concreteObjects[i].rotate(
							(physicObjects[i].angle - physicObjectsAngles[i]) *
								(180 / Math.PI)
						);
		...
		opentype.load("BO-2am.ttf", function(err, font) {
			captionText.forEach(function(text, i, arr) {
				...
				for (var i = 0; i < fontpaths.length; i++) {
					var paperpath = paper.project.importSVG(
						fontpaths[i].toSVG()
					);
					if (hope) {
...
function clearSimulation() {
	...
	var ground = Bodies.rectangle(-100, groundheight, 6400, 60, {
		isStatic: true
	});
...
function simulateText(text) {
	...
	opentype.load("BO-Midnight.ttf", function(err, font) {
		...
		var fontpaths = font.getPaths(
			text,
			(paper.view.viewSize.width - textwidth) / 2,
			0,
			500
		);
		...
		for (var i = 0; i < fontpaths.length; i++) {
			...
			var boundingbox = new Rectangle(
				[boundingboxData.x1, boundingboxData.y1],
				[
					boundingboxData.x2 - boundingboxData.x1,
					boundingboxData.y2 - boundingboxData.y1
				]
			);
...
function simulateSVG(svgstring) {
	...
	var svg = paper.project.importSVG(svgstring, {
		onLoad: handleSVG,
		onError: svgError,
		insert: true
	});
...
function addToWorld(path) {
	...
	var body = Bodies.fromVertices(
		path.bounds.center.x,
		path.bounds.center.y,
		vertices
	);
	if (body !== undefined) {
		//path with too few points can not be made to a body
...
function calcCrackShape(path, point) {
	...
	if (line.length > 100) {
		...
		var cutShape2 = calcCrackStencil(
			line2,
			n.rotate(rotation2).rotate(90),
			Math.floor(line.segments.length / 2)
		);
		cutShape2.insert(
			0,
			new Segment(cutShape.segments[idx].point, null, null)
		);
		cutShape2.add(
			new Segment(
				cutShape.segments[cutShape.segments.length - idx - 1].point,
				null,
				null
			)
		);
...
function calcCrackLine(path, point, direction, backset) {
	var line = new Path.Line(
		point.add(direction.multiply(backset)),
		point.add(direction.multiply(-500))
	);
...
function calcBreakShape(path) {
	...
	breakshape.add(
		new Segment(p1, null, n1.multiply(-state.getNextInt(80, 130)))
	);
	breakshape.add(
		new Segment(p2, n2.multiply(-state.getNextInt(80, 130)), null)
	);
...
function getPointInShape(shape) {
	var point = shape.bounds.topLeft.add(
		shape.bounds.size.multiply(Point.random())
	);

	while (!shape.contains(point)) {
		point = shape.bounds.topLeft.add(
			shape.bounds.size.multiply(Point.random())
		);
	}
	return point;
...
function breakPart(path) {
	...
	if (newpath._class == "CompoundPath") {
		...
		for (var i = newpath.children.length - 1; i > 0; i--) {
			//check only second paths, not main path
		...
		if (newpath.children.length > 1) {
			//still a compund path. has holes
			var item = newpath.clone();
			item.fillColor = concreteColor;
			item.insertAbove(newpath);
			newparts.push(item);
		} else {
			//no compoundpath - just take the single path
...
function animateLeaf(leafobject) {
	...
	var startTween = root1.tween(
		{ opacity: 0 },
		{ opacity: 1 },
		{ duration: state.rnd(150, 350), start: false }
	);
	startTween.start();
	startTween.then(function() {
		var tween = tweenstate.tween(
			{ strokeColor: leafColor, opacity: 1 },
			state.rnd(50, 150)
		);
		...
		tween.then(function() {
			...
			tween.then(function() {
				...
				tween.onUpdate = function(event) {
					circlesmall1.interpolate(
						circlesmall1,
						leafobject.children[3],
						event.factor
					);
				};
			});
		...
		var tween2 = tweenstate2.tween(
			{ strokeColor: leafColor, opacity: 1 },
			state.rnd(50, 150)
		);
		tween2.onUpdate = function(event) {
			tweenstate2.interpolate(
				outerstates[0],
				outerstates[1],
				event.factor
			);
					...
					tween2.then(function() {
						...
						tween2.then(function() {
							...
							tween2.onUpdate = function(event) {
								tweenstate2.interpolate(
									outerstates[5],
									leafobject.children[2],
									event.factor
								);
							};
							tween2.then(function() {
								tween2 = circlesmall2.tween(
									state.rnd(150, 350)
								);
								tween2.onUpdate = function(event) {
									circlesmall2.interpolate(
										circlesmall2,
										leafobject.children[4],
										event.factor
									);
								};
							});
...
function share() {
	urlstring =
		...
		endTime +
		"&caption=" +
		encodeURIComponent(captionText.toString()) +
		"&text=" +
		encodeURIComponent(simText);
